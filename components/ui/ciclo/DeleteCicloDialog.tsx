...
interface DeleteCicloProps {
  id: number;
}

export function DeleteCicloDialog({ id }: DeleteCicloProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/ciclo/${id}`, {
        method: "DELETE",
      });
      ...
      toast.success("Sala eliminada", {
        description: "El ciclo ha sido removida de la base de datos.",
      });
      ...
      toast.error("Error", {
        description: "No se pudo eliminar el ciclo. Inténtalo de nuevo.",
      });
      console.error(error);
    } finally {
      setIsDeleting(false);
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            ciclo con ID <span className="font-bold text-foreground">{id}</span>{" "}.
          </AlertDialogDescription>
        </AlertDialogHeader>
