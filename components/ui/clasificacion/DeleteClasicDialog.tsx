...
interface DeleteClasicProps {
  id: number;
}

export function DeleteClasicDialog({ id }: DeleteClasicProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/clasificacion/${id}`, {
        method: "DELETE",
      });
      ...
      toast.success("Sala eliminada", {
        description: "La clasificacion ha sido removida de la base de datos.",
      });
      ...
      toast.error("Error", {
        description: "No se pudo eliminar la clasificacion. Inténtalo de nuevo.",
      });
      console.error(error);
    } finally {
      setIsDeleting(false);
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            clasificacion con ID <span className="font-bold text-foreground">{id}</span>{" "}.
          </AlertDialogDescription>
        </AlertDialogHeader>
